...
grammar = """
    ...
    statement = import_statement | if_statement | while_statement | print_statement | function_statement | return_statement | brobare_statement | assignment_statement
    brobare_statement = "brobare"
...
def parse_simple_expression(tokens):
    token = tokens[0]
    if token["tag"] == "brobare":
        raise Exception(f"'brobare' cannot be used as an expression at position {token['position']}")
    """
    simple_expression = <number> | <string> | <identifier> | "(" expression ")" | "not" expression | "-" expression | function | object | array
    """
    token = tokens[0]
...
def parse_brobare_statement(tokens):
    """
    brobare_statement = "brobare"
    """
    assert tokens[0]["tag"] == "brobare", f"Expected 'brobare', got {tokens[0]}"
    return {"tag": "brobare"}, tokens[1:]


def parse_statement(tokens):
    tag = tokens[0]["tag"]  # Ensure tag is properly extracted
    if tag == "brobare":
        return parse_brobare_statement(tokens)
